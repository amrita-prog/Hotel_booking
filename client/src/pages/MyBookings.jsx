          ...
          <div key={booking._id} className='grid grid-cols-1 md:grid-cols-[3fr_2fr_1fr] w-full border-b border-gray-300 py-6 first:border-t'>
            ...
            <div className='flex flex-col items-start justify-center pt-3'>
              <div className='flex items-center mt-3 md:mt-0 gap-2'>
                ...
                <span className={`text-base font-medium ${booking.isPaid ? "text-green-600" : "text-red-500"}`}>
                  {booking.isPaid ? "Paid" : "Unpaid"}
                </span>
              </div>
              {!booking.isPaid && (
                <button className='px-4 py-1.5 mt-4 text-xs border border-gray-400 rounded-full hover:bg-blue-500 hover:text-white transition-all cursor-pointer'>
                  Pay Now
                </button>
              )}
            </div>
